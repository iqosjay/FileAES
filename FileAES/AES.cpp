 ...
 *
 * "Only the first some of these constants are actually used ¨C up to rcon[10]
...
void Aes::KeyExpansion(uint8_t* RoundKey, const uint8_t* Key) {
  unsigned i, j, k;
...
void Aes::InitAesCtx(AesCtx* ctx, const uint8_t* key) {
  Aes::KeyExpansion(ctx->RoundKey, key);
}
#if (defined(CBC) && (CBC == 1)) || (defined(CTR) && (CTR == 1))
void Aes::AESInitCtxIv(AesCtx* ctx, const uint8_t* key, const uint8_t* iv) {
  Aes::KeyExpansion(ctx->RoundKey, key);
  std::memcpy(ctx->Iv, iv, AES_BLOCKLEN);
}

void Aes::AESCtxSetIv(AesCtx* ctx, const uint8_t* iv) {
  std::memcpy(ctx->Iv, iv, AES_BLOCKLEN);
}
#endif
...
void Aes::AesCBCEncryptBuffer(AesCtx* ctx, uint8_t* buf, uint32_t length) {
  uintptr_t i;
  uint8_t* Iv = ctx->Iv;
...
void Aes::AesCBCDecryptBuffer(AesCtx* ctx, uint8_t* buf, uint32_t length) {
  uintptr_t i;
...
void Aes::AesCTRXcryptBuffer(AesCtx* ctx, uint8_t* buf, uint32_t length) {
